(function (global, factory) {
    ...
    function isValid(m) {
        if (m._isValid == null) {
            ...
            let isNowValid = !isNaN(m._d.getTime()) && flags.overflow < 0 && !flags.empty && !flags.invalidMonth && !flags.invalidWeekday && !flags.weekdayMismatch && !flags.nullInput && !flags.invalidFormat && !flags.userInvalidated && (!flags.meridiem || (flags.meridiem && parsedParts));
    ...
    function compareArrays(array1, array2, dontConvert) {
        let len = Math.min(array1.length, array2.length), lengthDiff = Math.abs(array1.length - array2.length),
            diffs = 0, i;
        for (i = 0; i < len; i++) {
    ...
    function longDateFormat(key) {
        let format = this._longDateFormat[key], formatUpper = this._longDateFormat[key.toUpperCase()];
    ...
    function zeroFill(number, targetLength, forceSign) {
        let absNumber = '' + Math.abs(number), zerosToFill = targetLength - absNumber.length, sign = number >= 0;
    ...
    let match6 = /[+-]?\d{6}/;
    let match1to2 = /\d\d?/;
    let match3to4 = /\d\d\d\d?/;
    let match5to6 = /\d\d\d\d\d\d?/;
    let match1to3 = /\d{1,3}/;
    let match1to4 = /\d{1,4}/;
    let match1to6 = /[+-]?\d{1,6}/;
    let matchUnsigned = /\d+/;
    let matchSigned = /[+-]?\d+/;
    let matchOffset = /Z|[+-]\d\d:?\d\d/gi;
    let matchShortOffset = /Z|[+-]\d\d(?::?\d\d)?/gi;
    let matchTimestamp = /[+-]?\d+(\.\d{1,3})?/;
    ...
    function firstWeekOffset(year, dow, doy) {
        let fwd = 7 + dow - doy, fwdlw = (7 + createUTCDate(year, 0, fwd).getUTCDay() - dow) % 7;

        return -fwdlw + fwd - 1;
    }

    function dayOfYearFromWeeks(year, week, weekday, dow, doy) {
        let localWeekday = (7 + weekday - dow) % 7, weekOffset = firstWeekOffset(year, dow, doy),
    ...
    function weeksInYear(year, dow, doy) {
        let weekOffset = firstWeekOffset(year, dow, doy), weekOffsetNext = firstWeekOffset(year + 1, dow, doy);
    ...
    let isoDates = [['YYYYYY-MM-DD', /[+-]\d{6}-\d\d-\d\d/], ['YYYY-MM-DD', /\d{4}-\d\d-\d\d/], ['GGGG-[W]WW-E', /\d{4}-W\d\d-\d/], ['GGGG-[W]WW', /\d{4}-W\d\d/, false], ['YYYY-DDD', /\d{4}-\d{3}/], ['YYYY-MM', /\d{4}-\d\d/, false], ['YYYYYYMMDD', /[+-]\d{10}/], ['YYYYMMDD', /\d{8}/], ['GGGG[W]WWE', /\d{4}W\d{3}/], ['GGGG[W]WW', /\d{4}W\d{2}/, false], ['YYYYDDD', /\d{7}/]];

    let isoTimes = [['HH:mm:ss.SSSS', /\d\d:\d\d:\d\d\.\d+/], ['HH:mm:ss,SSSS', /\d\d:\d\d:\d\d,\d+/], ['HH:mm:ss', /\d\d:\d\d:\d\d/], ['HH:mm', /\d\d:\d\d/], ['HHmmss.SSSS', /\d\d\d\d\d\d\.\d+/], ['HHmmss,SSSS', /\d\d\d\d\d\d,\d+/], ['HHmmss', /\d\d\d\d\d\d/], ['HHmm', /\d\d\d\d/], ['HH', /\d\d/]];

    let aspNetJsonRegex = /^\/?Date\((-?\d+)/i;

    function configFromISO(config) {
        let i, l, string = config._i, match = extendedIsoRegex.exec(string) || basicIsoRegex.exec(string), allowTime,
    ...
    function extractFromRFC2822Strings(yearStr, monthStr, dayStr, hourStr, minuteStr, secondStr) {
        let result = [untruncateYear(yearStr), defaultLocaleMonthsShort.indexOf(monthStr), parseInt(dayStr, 10), parseInt(hourStr, 10), parseInt(minuteStr, 10)];
    ...
    function configFromStringAndFormat(config) {
        ...
        let string = '' + config._i, i, parsedInput, tokens, token, skipped, stringLength = string.length,
    ...
    let prototypeMin = deprecate('moment().min is deprecated, use moment.max instead. http://momentjs.com/guides/#/warnings/min-max/', function () {
    ...
    let prototypeMax = deprecate('moment().max is deprecated, use moment.min instead. http://momentjs.com/guides/#/warnings/min-max/', function () {
        let other = createLocal.apply(null, arguments);
        if (this.isValid() && other.isValid()) {
            return other > this ? this : other;
        } else {
    ...
    function Duration(duration) {
        let normalizedInput = normalizeObjectUnits(duration), years = normalizedInput.year || 0,
    ...
    function createDuration(input, key) {
        let duration = input, match = null, sign, ret, diffRes;
    ...
    function addSubtract(mom, duration, isAdding, updateOffset) {
        let milliseconds = duration._milliseconds, days = absRound(duration._days), months = absRound(duration._months);
    ...
    function calendar$1(time, formats) {
        let now = time || createLocal(), sod = cloneWithOffset(now, this).startOf('day'),
    ...
    let lang = deprecate('moment().lang() is deprecated. Instead, use moment().localeData() to get the language configuration. Use moment().locale() to change languages.', function (key) {
        if (key === undefined) {
    ...
    function listWeekdaysImpl(localeSorted, format, index, field) {
        ...
        let locale = getLocale(), shift = localeSorted ? locale._week.dow : 0;
    ...
    function relativeTime$1(posNegDuration, withoutSuffix, locale) {
        ...
        let a = seconds <= thresholds.ss && ['s', seconds] || seconds < thresholds.s && ['ss', seconds] || minutes <= 1 && ['m'] || minutes < thresholds.m && ['mm', minutes] || hours <= 1 && ['h'] || hours < thresholds.h && ['hh', hours] || days <= 1 && ['d'] || days < thresholds.d && ['dd', days] || months <= 1 && ['M'] || months < thresholds.M && ['MM', months] || years <= 1 && ['y'] || ['yy', years];
